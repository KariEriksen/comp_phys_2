...
for gamma in gamma_vals:
    ...
    for i in sims_n_str:
        ...
        t_filename = "../data/g_data/"+filename
...
time_data = np.loadtxt("../data/g_data/time_iter.csv")
...
plt.title(r"Simulation of $N = 2$, $D = 2$ using brute force metropolis algorithm - with interaction")
#plt.savefig("naive_mh_testing.pdf")
figname = "interactive_naive"
...
if fast == False:
    with open(figname+".txt", "w") as outfile:
        ...
        for i in range(len(blocking_data)):
            var = np.average(np.sqrt(blocking_data[i][60:]))
            energy = np.average(energies[i][60:])
